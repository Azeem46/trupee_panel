...
class FnoEquityReport extends React.Component {
  state = {
    rowData: [],
    StartDate: "",
    EndDate: "",
  ...
  alltradeList = () => {
    axiosConfig.get(`/admin/reportTrade?type=Equity`).then((response) => {
  ...
  submitHandler = (e) => {
    ...
    axiosConfig
      .get(
        `/admin/reportTrade?type=Equity&start=${this.state.StartDate}&end=${this.state.EndDate}`
      )
      .then((response) => {
...
export default FnoEquityReport;
